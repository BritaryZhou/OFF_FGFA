...
def train_net(args, ctx, pretrained, pretrained_flow, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    feat_sym = sym.get_internals()['rpn_cls_score_output'] # rpn_cls_score

    # setup multi-gpu
    batch_size = len(ctx) # equals gpu_num=4
    input_batch_size = config.TRAIN.BATCH_IMAGES * batch_size #1*4 = 4
    ...
    for child_metric in [eval_metric, cls_metric, bbox_metric]:   #rpn_eval_metric, rpn_cls_metric, rpn_bbox_metric,
    ...
    print "mod.data_shapes..:", mod.data_shapes
    print "mod.label_shapes..:", mod.label_shapes
    print "mod.params..:", mod.get_params()
    logger.info('mod.data_shapes:{}\n'.format(pprint.pformat(mod.data_shapes)))
    logger.info('mod.label_shapes:{}\n'.format(pprint.pformat(mod.label_shapes)))
    logger.info('mod.params:{}\n'.format(pprint.pformat(mod.get_params())))
