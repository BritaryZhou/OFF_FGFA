...
def train_net(args, ctx, pretrained, pretrained_flow, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    # train
    #print('******************************************')
    #mx.viz.plot_network(mod,title="Network.pdf",save_format='pdf').view()
    #mx.viz.plot_network(mod).view()  
   # print('output_num:',len(mod.get_outputs()))
    #logger.info('network:\n'.format(pprint.pformat(mod)))
    mod.fit(train_data, eval_metric=eval_metrics, epoch_end_callback=epoch_end_callback,
            batch_end_callback=batch_end_callback, kvstore=config.default.kvstore,
            optimizer='sgd', optimizer_params=optimizer_params,
            arg_params=arg_params, aux_params=aux_params, begin_epoch=begin_epoch, num_epoch=end_epoch)
    #print('output_num:',len(mod.get_outputs()))
    #logger.info('output_num:\n'.format(pprint.pformat(len(mod.get_outputs()))))
