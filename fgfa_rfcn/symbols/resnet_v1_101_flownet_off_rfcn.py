...
class resnet_v1_101_flownet_off_rfcn(Symbol):
    ...
    def get_train_symbol(self, cfg):
       ...
       # OFF-subnet added by zy #####################################3
        off_pre_conv = mx.symbol.Convolution(name='off_pre_conv', data=select_conv_feat, num_filter=256, pad=(0, 0), kernel=(1, 1), stride=(1, 1))
        off_pre_relu = mx.symbol.Activation(name='off_pre_relu', data=off_pre_conv, act_type='relu')
        ...
        #slice_diff = mx.symbol.sum(data = select_conv_feat_slice_last - select_conv_feat_slice_first, axis=1)
        slice_diff = select_conv_feat_slice_last - select_conv_feat_slice_first
        #print mx.symbol.shape_array(slice_diff)
        #self.logger.info('slice_off_shape:{}\n'.format(pprint.pformat(mx.symbol.shape_array(slice_diff))))
        slice_diff_reshape = mx.symbol.reshape(slice_diff, shape=(1, 256, 0, 0))
        sobel_input = mx.symbol.reshape(select_conv_feat, shape=(3, -1, 0, 0))  # frame_size*batch_size
        ...
        off_concat = mx.symbol.Concat(slice_diff_reshape, sobel_dropout)
        off_concat_reshape = mx.symbol.reshape(off_concat, shape=(0, 1024, 0, 0))

        # residual block
        off_res_conv1 = mx.sym.Convolution(name='off_res_conv1', data=off_concat_reshape, kernel=(1, 1), pad=(0, 0), stride=(1,1), num_filter=256)
        ...
        off_res_relu1 = mx.symbol.Activation(name='off_res_relu1', data=off_res_bn1, act_type='relu')
        off_res_conv2 = mx.sym.Convolution(name='off_res_conv2', data=off_res_relu1, kernel=(3, 3), pad=(1, 1), stride=(1,1), num_filter=256)
        ...
        off_res_relu2 = mx.symbol.Activation(name='off_res_relu2', data=off_res_bn2, act_type='relu')
        off_res_conv3 = mx.sym.Convolution(name='off_res_conv3', data=off_res_relu2, kernel=(1, 1), pad=(0, 0), stride=(1,1), num_filter=1024)
        ...
        off_res_add = mx.symbol.broadcast_add(name='off_res_add', *[off_concat_reshape, off_res_bn3])

        ###########################################################

        conv_feats = mx.sym.SliceChannel(off_res_add, axis=1, num_outputs=2)
    ...
    def init_weight(self, cfg, arg_params, aux_params):
        ...
        ################################## off_res_init
        arg_params['off_pre_conv_weight'] = mx.random.normal(0, self.get_msra_std(self.arg_shape_dict['off_pre_conv_weight']), shape=self.arg_shape_dict['off_pre_conv_weight'])
        arg_params['off_pre_conv_bias'] = mx.nd.ones(shape=self.arg_shape_dict['off_pre_conv_bias'])*0.2
        ...
        ky = kx.T
        arg_params['off_res_conv1_weight'] = mx.random.normal(0, self.get_msra_std(self.arg_shape_dict['off_res_conv1_weight']), shape=self.arg_shape_dict['off_res_conv1_weight'])
        arg_params['off_res_conv1_bias'] = mx.nd.ones(shape=self.arg_shape_dict['off_res_conv1_bias']) * 0.2
        arg_params['off_res_conv2_weight'] = mx.random.normal(0, self.get_msra_std(self.arg_shape_dict['off_res_conv2_weight']), shape=self.arg_shape_dict['off_res_conv2_weight'])
        arg_params['off_res_conv2_bias'] = mx.nd.ones(shape=self.arg_shape_dict['off_res_conv2_bias']) * 0.2
        arg_params['off_res_conv3_weight'] = mx.random.normal(0, self.get_msra_std(self.arg_shape_dict['off_res_conv3_weight']), shape=self.arg_shape_dict['off_res_conv3_weight'])
        arg_params['off_res_conv3_bias'] = mx.nd.ones(shape=self.arg_shape_dict['off_res_conv3_bias']) * 0.2
