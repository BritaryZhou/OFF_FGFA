...
from rpn.rpn import get_rpn_testbatch, get_rpn_triple_batch, assign_anchor, get_rpn_batch, get_rpn_pair_batch
from rpn.rpn import get_rpn_pair_testbatch
...
class TestLoader(mx.io.DataIter):
    def __init__(self, roidb, config, batch_size=1, shuffle=False,
        ...
        if config.TRAIN.E2E_NAME == "off" :
            print "e2e_name = off"
            self.data_name = ['data', 'data_ref', 'im_info', 'data_cache', 'feat_cache']
        else:
    ...
    def get_init_batch(self):
        ...
        if self.cfg.TRAIN.E2E_NAME == 'off':
            extend_data = [{'data': data[0]['data'] ,
                            'data_ref': data[0]['data_ref'],
                            'im_info': data[0]['im_info'],
                            'data_cache': np.zeros((1, 3, max([v[0] for v in self.cfg.SCALES]), max([v[1] for v in self.cfg.SCALES]))),
                            'feat_cache': np.zeros((2, self.cfg.network.FGFA_FEAT_DIM,
            ...
            extend_data = [{'data': data[0]['data'] ,
                        'im_info': data[0]['im_info'],
                        'data_cache': np.zeros((2*self.cfg.TEST.KEY_FRAME_INTERVAL+1, 3, max([v[0] for v in self.cfg.SCALES]), max([v[1] for v in self.cfg.SCALES]))),
                        'feat_cache': np.zeros((2*self.cfg.TEST.KEY_FRAME_INTERVAL+1, self.cfg.network.FGFA_FEAT_DIM,
                                                np.ceil(max([v[0] for v in self.cfg.SCALES]) / feat_stride).astype(np.int),
                                                np.ceil(max([v[1] for v in self.cfg.SCALES]) / feat_stride).astype(np.int)))}]
        self.data = [[mx.nd.array(extend_data[i][name]) for name in self.data_name] for i in xrange(len(data))]
        self.im_info = im_info
...
class AnchorLoader(mx.io.DataIter):
    ...
    def __init__(self, feat_sym, roidb, cfg, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
        ...
        if config.TRAIN.END2END and config.TRAIN.E2E_NAME == "base" :
            print "e2e_name = base"
            ...
            print "e2e_name = off"
            self.data_name = ['data', 'data_ref', 'im_info', 'gt_boxes']
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        ## assign_anchor. return label, bbox_target, bbox_weight
