...
class Predictor(object):
    ...
    def predict(self, data_batch):
        ...
        # print [dict(zip(self._mod.output_names, _)) for _ in zip(*self._mod.get_outputs(merge_multi_context=False))]
...
def get_resnet_output(predictor, data_batch, data_names, cfg):
    ...
    if cfg.TRAIN.E2E_NAME == 'base' or cfg.TRAIN.E2E_NAME == 'off':
        ...
        key_output_name = 'conv_embed_output'

    if output_all[0].has_key(key_output_name):
        feat = output_all[0][key_output_name]
    else:
        feat = None
    # print 'res_feat_size{}'.format(feat.asnumpy().shape)
    # cv2.imwrite('/mnt/lustre/sunshuyang/FGFA-bj30/FGFA-725v/data_input.jpg', data_dict_all[0]['data'][0][0].asnumpy())
    # cv2.imwrite('/mnt/lustre/sunshuyang/FGFA-bj30/FGFA-725v/data_ref_input.jpg', data_dict_all[0]['data_ref'][0][0].asnumpy())
    # data_img = data_dict_all[0]['data'][0][0].asnumpy()
    # height, width = data_img.shape[:2]
    # feat1 = feat[0][0].asnumpy()
    # feat1_rs = cv2.resize(feat1, (width, height))
    # for i in range(height):
    #     for j in range(width):
    #         feat1_rs[i][j] = feat1_rs[i][j] * data_img[i][j]
    # cv2.imwrite('/mnt/lustre/sunshuyang/FGFA-bj30/FGFA-725v/feat_output1.jpg', feat1_rs)
    # data_ref_img = data_dict_all[0]['data_ref'][0][0].asnumpy()
    # height, width = data_ref_img.shape[:2]
    # feat2 = feat[1][0].asnumpy()
    # feat2_rs = cv2.resize(feat2, (width, height))
    # for i in range(height):
    #     for j in range(width):
    #         feat2_rs[i][j] = feat2_rs[i][j] * data_ref_img[i][j]
    # cv2.imwrite('/mnt/lustre/sunshuyang/FGFA-bj30/FGFA-725v/feat_output2.jpg', feat2_rs)
...
def pred_eval(gpu_id, feat_predictors, aggr_predictors, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    ...
    assert vis or not test_data.shuffle
    data_names = [k[0] for k in test_data.provide_data[0]] # data, img_info, data_cache, feat_cache
    ...
    for im_info, key_frame_flag, data_batch in test_data:
        ...
        if cfg.TRAIN.E2E_NAME == 'off':
            ...
            if key_frame_flag == 0:
                ...
                if logger:
                    logger.info('key_frame_interval:{}\n'.format(key_frame_interval))
            ...
            #################################################
                ...
                if len(data_list) < all_frame_interval - 1: # off:1
                    ...
                    feat_list.append(feat)
                    prepare_data(data_list, feat_list, data_batch, logger)
                    print 'pred_eval,flag=2:{}'.format(data_batch.data[0][-2].shape)
                    if logger:
                        logger.info('pred_eval,flag=2:{}'.format(data_batch.data[0][-2].shape))
                    pred_result = im_detect(aggr_predictors, data_batch, data_names, scales, cfg)
...
def prepare_data(data_list, feat_list, data_batch, logger):
