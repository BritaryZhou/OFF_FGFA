...
def vid_eval_motion(multifiles, detpath, annopath, imageset_file, classname_map, annocache, motion_iou_file, motion_ranges, area_ranges, ovthresh=0.5):
    ...
    gt_img_ids = [int(x[1]) for x in lines]  #176126
    classhash = dict(zip(classname_map, range(0,len(classname_map)))) # 31
   ...
   # load annotations from cache
    ...
    # sort by img_ids
    if obj_bboxes.shape[0] > 0: # 36475606
    ...
    for i in range(0, len(img_ids)):
        if i == len(img_ids)-1 or img_ids[i+1] != id:
            ...
            print 'sorted_inds:{}'.format(sorted_inds)
    ...
    ov_all = [None] * num_imgs # store the iou results of each generate results and corresponding gt bbox.
    # extract objects in :param classname:
    npos = np.zeros(len(classname_map)) # count for gt nums of each class
    for index, rec in enumerate(recs): # annotations.
        id = rec['img_ids']
        gt_labels = rec['label']
        gt_bboxes = rec['bbox']
    ...
    for motion_range_id, motion_range in enumerate(motion_ranges):
        for area_range_id, area_range in enumerate(area_ranges):
            ...
            print ap[motion_range_id][area_range_id]
